...
namespace login
{
    public partial class AdminSubjectsList : Form
    {
        ...
        private void btnGuardar_Click(object sender, EventArgs e)
        {
            ...
            dgvMateria.Refresh();
            data.cargarEstudiantes();
            List<Estudiante> students = data.obtenerEstudiantes();
            Estudiante value = new Estudiante();
            value = students.Find(x => x.usuario == usuarioActual);
            foreach (Materia mate in value.Materias)
            {
                DataGridViewRow nuevaFila = new DataGridViewRow();
                DataGridViewCell nuevaCelda = new DataGridViewTextBoxCell();
        ...
        private void txbCalificacion_Validating(object sender, CancelEventArgs e)
        {

            string errormsg = "";
            Validacion valid = new Validacion();
            if (!valid.validarNumeros(txbCalificacion.Text, "calificacion", out errormsg))
            {
                e.Cancel = true;
                this.errorProvider1.SetError(txbCalificacion, errormsg);
            }
            else
            {
                e.Cancel = false;
