...
namespace login
{
    public partial class AgregarAsignatura : Form
    {
        ...
        private void tbCreditos_Validating(object sender, CancelEventArgs e)
        {

            string errormsg = "";
            Validacion valid = new Validacion();
            if (!valid.validarNumeros(tbCreditos.Text, "credito",out errormsg))
            {
                e.Cancel = true;
                this.errorProvider1.SetError(tbCreditos, errormsg);
            }
            else
            {
                e.Cancel = false;
